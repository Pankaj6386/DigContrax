...
class ChangePassword extends Component {
  ...
  _updatePassword() {
    ...
    fetch(
      config.BASE_URL + 'change-password?api_token=' + config.currentToken,
      {
        method: 'POST',
        headers: {
        ...
        <Container>
          ...
          <Content>
            <View style={{margin: 10}}>
              ...
              <H3 style={{fontFamily: 'RacingSansOne-Regular', marginTop: 10}}>
			  {this.props?.t('User Information')}
              </H3>
              <View>
                <Label style={{marginTop: 10}}>{this.props?.t('Current Password')}</Label>
                <Item regular style={{backgroundColor: '#fff', marginTop: 5}}>
                  <Input
                    defaultValue={this.state.old_password}
                    onChangeText={text =>
                      this.setState({old_password: text.trim()})
                    }
                    secureTextEntry={true}
                  />
                </Item>

                <Label style={{marginTop: 10}}>{this.props?.t('New Password')}</Label>
                <Item regular style={{backgroundColor: '#fff', marginTop: 5}}>
                  <Input
                    defaultValue={this.state.new_password}
                    onChangeText={text =>
                      this.setState({new_password: text.trim()})
                    }
                    secureTextEntry={true}
                  />
                </Item>

                <Label style={{marginTop: 10}}>{this.props?.t('Confirm New Password')}</Label>
                <Item regular style={{backgroundColor: '#fff', marginTop: 5}}>
                  <Input
                    defaultValue={this.state.confirm_password}
                    onChangeText={text =>
                      this.setState({confirm_password: text.trim()})
                    }
                    secureTextEntry={true}
                  />
                </Item>
              ...
              <Button
                ...
                style={{backgroundColor: '#E05439', marginTop: 25}}>
